...
public class everland {
	...
	public static void main(String[] args) {
		...
		do {
			...
			saveTicket[i] = ticket;
			saveTicketSort[i] = ticketSort;
			saveNumOfPeople[i] = numOfPeople;
			saveTotalPrice[i] = totalPrice;
			saveDiscount[i] = discount;
		...
		for (int index = 0; index < i; index++) {
			sumTotal += saveTotalPrice[index];
			System.out.printf("%s X %d %d * %d번 우대 적용\n", saveTicketSort[index], saveNumOfPeople[index],
					saveTotalPrice[index], saveDiscount[index]);

		}
		System.out.println("========================================");
		System.out.printf("총 금액은 %d입니다.", sumTotal);
		sc.close();
	}
